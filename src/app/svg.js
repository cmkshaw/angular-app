...
$(document).ready(function() { 
	...
	var s = Snap("#logo-anim-desktop");
	var delayTime = 1000;

	$(".header-title .pink").hide();
    $(".header-title .blue").hide();

	function logoAnimation(){
		Snap.load("./img/Instascreen_logo.svg", onLogoLoad);
		$(".header-title .pink").hide().delay(500).fadeIn(500);
		$(".header-title").css("display", "block");
     	$(".header-title .blue").hide().delay(700).fadeIn(500);
	}

	function onLogoLoad(f) {
	    // Note that we traversre and change attr before SVG
	    // is even added to the page
	    //f.selectAll("polygon[fill='#E34497']").attr({fill: "#fff"});

	    bigrect = f.select('#big-rect');
	    pinkrects = f.selectAll("polygon[fill='#E34497']");
	    darkbluerects = f.selectAll("polygon[fill='#33588E']");
	    lightbluerects = f.selectAll("polygon[fill='#80A7BC']");

	    //bigrect.attr({"fill-opacity": 0.5});
	    transformElements(pinkrects);
	    transformElements(darkbluerects);
	    transformElements(lightbluerects);

	    bigrect.transform('s0r0,150,150');
		bigrect.animate({ transform: 's1r0,150,150'/*, 'fill-opacity': 1.0*/}, 600, mina.elastic );
	  	
	  	setTimeout(function() {
	  		animateElastic(darkbluerects);
	  	}, 100);
	  	setTimeout(function() {
	  		animateElastic(lightbluerects);
	  	}, 300);
	  	setTimeout( function() {
	  		animateElastic(pinkrects);
	  	}, 500);
	    s.append(f);

	}

	var transformElements = function(elements) {		
		for (var i = 0; i < elements.length; i++) {
	    	elements[i].transform('s0r0,150,150');
	    }
	}
	var animateElastic = function(elements) {
		for (var i = 0; i < elements.length; i++) {
	    	elements[i].animate( { transform: 's1r0,150,150' }, 1000, mina.elastic );
	    }
	}
	...
	//Conditionally allow animation
	 if (isFirefox == true){
	 	$("#logo-anim-desktop").hide().html("<img src='../img/Instascreen_logo.svg'>").delay(500).fadeIn(500);;
	 	$(".header-title .pink").hide().delay(600).fadeIn(500);
	 	$(".header-title").css("display", "block");
	  	$(".header-title .blue").hide().delay(700).fadeIn(500);
		return;
	 }
	 else if (ieVersion == 8){
	 	$("#logo-anim-desktop").hide().html("<img src='img/instascreen_logo.png'>").delay(500).fadeIn(500);;
	 	$(".header-title .pink").hide().delay(600).fadeIn(500);
	 	$(".header-title").css("display", "block");
	  	$(".header-title .blue").hide().delay(700).fadeIn(500);
	  	//IE8 HTC Hacks for background-size
		$('.tiff-tab').prop('class', 'tiff-tab');
		$('.danier-tab').prop('class', 'danier-tab');
		$('.starwood-tab').prop('class', 'starwood-tab');
		$('.artscience').prop('class', 'artscience');
		return;
	 }
	 else{
	 	setTimeout(logoAnimation, delayTime);
